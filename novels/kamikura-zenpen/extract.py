...
# 第一次访问的网址
START_URL = "http://bfpblog.bakafire.main.jp/?eid=606"
...
def load_state():
    if os.path.exists(STATE_FILE):
        try:
            ...
            return (
                data["next_url"],
                int(data["file_number"])
            )

        except Exception as e:
            print("[WARN] 状态文件读取失败：", e)

    return START_URL, START_FILE_NUMBER
...
{images_html}
        ...
        <div class="floating-controls">
            <button onclick="window.location.href='{file_number - 1:02d}.html'" class="home-button">上一話</button>
            ...
            <button onclick="window.location.href='{file_number + 1:02d}.html'" class="next-button">下一話</button>
        </div>
    </div>
...
def main():
    ...
    if next_url:
        ...
        print(
            f"[INFO] 下次文件："
            f"{file_number + 1:02d}{FILE_SUFFIX}"
        )
